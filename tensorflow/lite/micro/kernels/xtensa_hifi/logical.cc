...
TfLiteStatus LogicalImpl(TfLiteContext* context, TfLiteNode* node,
  ...
  if (tflite::micro::HaveSameShapes(input1, input2)) {
    ...
    if (func == LogicalAnd) {
          err = xa_nn_elm_logicaland_boolxbool_bool(output_data_ptr,
                                                  input1_data_ptr,
                                                  input2_data_ptr,
                                                  flat_size);
          CHECK_ERR_HIFI_NNLIB_KER(err, "xa_nn_elm_logicaland_boolxbool_bool failed");
    } else if (func == LogicalOr) {
          err = xa_nn_elm_logicalor_boolxbool_bool(output_data_ptr,
                                                  input1_data_ptr,
                                                  input2_data_ptr,
                                                  flat_size);
          CHECK_ERR_HIFI_NNLIB_KER(err, "xa_nn_elm_logicalor_boolxbool_bool failed");
    } else {
          err = 1;
          CHECK_ERR_HIFI_NNLIB_KER(err, "wrong function called");
    }   
#else   
